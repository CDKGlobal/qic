...
public class Retriever {
	...
	private static Set<DataPoint> privateGetData(String startDate, String endDate){
		...
		String absolutePath = home
				+ "/Perforce/chenc_sea-chenc-m_qic/Playpen/QIC2/";
		try {
			dataList = mapper.readValue(new File(absolutePath + startDate
					+ "/17271/files.json"), new TypeReference<List<WebData>>() {
			});
				...
				if (!dataList.isEmpty() && "CLA".equals(file.getQualifier())) {
					...
					if (map.containsKey(key)) {
						double val0 = file.getMsr().get(0).getVal()
								- map.get(key)[0];
						// if(val0!=0 || val1!=0){
						dataSet.add(new DataPoint(file.getName(), Math
								.abs(val0), map.get(key)[1]));
						// }
					} else {
